...
public interface InstituteRepository extends JpaRepository<Institute, String> {
    ...
    @Query(value = """
        ...
        LEFT JOIN courses c ON c.identifier = ic.course_identifier
        LEFT JOIN institute_facilities f ON f.institute_identifier = i.identifier
        WHERE i.is_active = true
          AND (:query IS NULL OR i.name ILIKE CONCAT('%', :query, '%') OR c.name ILIKE CONCAT('%', :query, '%') OR i.tagline ILIKE CONCAT('%', :query, '%') OR i.description ILIKE CONCAT('%', :query, '%'))
